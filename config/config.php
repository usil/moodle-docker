...
$CFG->dbpass    = getenv('MYSQL_USER_PASSWORD');   // your database password
...
//   Redis session handler (requires redis server and redis extension):
      ...
      if (getenv('REDIS_HOST')!=null){
        ...
        if (getenv('REDIS_PASSWORD')!=null){$CFG->session_redis_auth = getenv('REDIS_PASSWORD');} // Optional, default is don't set one.
...
// it may be also necessary to enable this when using port forwarding.
      $CFG->reverseproxy = json_decode(getenv('MOODLE_REVERSE_PROXY'));
//
// Enable when using external SSL appliance for performance reasons.
// Please note that site may be accessible via http: or https:, but not both!
      $CFG->sslproxy = json_decode(getenv('MOODLE_SSLPROXY'));
//
